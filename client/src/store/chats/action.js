...
const chatsSlice = createSlice({
  ...
  reducers: {
    ...
    chatUpdated(state, action) {
      const { id, title, content, contact } = action.payload
      console.log(`action.payload: ${action.payload}`)
      const existingChat = state.find(post => post.id === id)
      if (existingChat) {
        existingChat.title = title
        existingChat.content = content
        existingChat.contact = contact
      }
    }
  }
})

export const { chatAdded, chatUpdated } = chatsSlice.actions
